...
export function CategoryCard({ title, image, slug }: Props) {
  return (
    <Link href={`/products/${slug}`} className="group">
      <div className="flex flex-col items-center text-center  rounded-lg shadow-sm transition p-4">
        <Image
          src={image}
          alt={title}
          width={148}
        ...
        <span className="text-sm font-medium text-gray-700 group-hover:gray-500">{title}</span>
      </div>
    </Link>
  );
}
