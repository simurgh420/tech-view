...
export async function addCartItemRequest(payload: CartItemPayload): Promise<CartItem> {
...
export async function removeCartItemRequest(id: string): Promise<{ success: boolean }> {
  const res = await axios.delete(`/api/cart/${id}`);
  return res.data;
}

export async function clearCartRequest(cartId: string): Promise<{ success: boolean }> {
